...
class ClickUp:
    ...
    async def create_threaded_comment(
        ...
        """Add a threaded comment to an existing comment."""
        data = {
            "comment_text": comment_text,
            "notify_all": str(notify_all).lower(),
        }
    ...
    async def create_task_attachment(
        ...
        Args:
            task_id: ID of the task to attach the file to (uses the one set by task() if not provided)
        ...
        Raises:
            ValueError: If neither file_path nor file_data is provided, or if file_name is missing when using file_data
            ResourceNotFound: If the task doesn't exist
            ValidationError: If the request data is invalid
            ClickUpError: For other API errors
        """
        ...
        # Prepare the request
        url = f"{self.base_url.rstrip('/')}/task/{task_id}/attachment"
